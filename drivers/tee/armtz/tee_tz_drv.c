...
static u32 handle_rpc(struct tee_tz *ptee, struct smc_param *param, uintptr_t parg32)
{
	...
	case TEESMC_ST_RPC_FUNC_ALLOC_PAYLOAD:
		...
		if (IS_ERR_OR_NULL(shm)) {
			dev_err(ptee->tee->dev, "%s: > err cmd:%08llx  IS_ERR_OR_NULL\n", __func__, param->a0);
			param->a1 = 0;
		...
		if (cookie < 0) {
			dev_err(ptee->tee->dev, "%s: > err cmd:%08llx get cookie err\n", __func__, param->a0);
	...
	case TEESMC_ST_RPC_FUNC_FREE_PAYLOAD:
		dev_dbg(ptee->tee->dev, "%s: > TEESMC_ST_RPC_FUNC_FREE_PAYLOAD, cookie %lld\n", __func__, param->a1);
			...
			uintptr_t parg32, struct teesmc32_arg *arg32)
{
	...
	param.a1 = parg32;
	e_lock_teez(ptee);
//	enable_clock(MT_CG_INFRA_TRNG, "TRNG");
	while (true) {
		__flush_dcache_area(ptee->shm_vaddr, ptee->shm_size);

		param.a0 = funcid;
		do {
#ifndef SMP_ENABLE
			work_on_cpu(0, (long (*)(void*))tee_smc_call, (void*)&param);
			//smp_call_function_single(0, (void (*)(void*))tee_smc_call, (void*)&param, 1);
#else
			tee_smc_call(&param);
#endif
...
//	disable_clock(MT_CG_INFRA_TRNG, "TRNG");
	e_unlock_teez(ptee);
...
static int configure_shm(struct tee_tz *ptee)
{
	...
	if (ptee->shm_cached)
		ptee->shm_vaddr = ioremap_cache(ptee->shm_paddr, ptee->shm_size);
	else
		ptee->shm_vaddr = ioremap_nocache(ptee->shm_paddr, ptee->shm_size);

	printk(KERN_ERR "shm ioremap %p\n", ptee->shm_vaddr);
...
//	pr_debug("- dev->parent=%p\n", dev->ctx);
	...
	if (of_machine_is_compatible("sprd,sc9850s-2h10-native")) {
		log_paddr = (unsigned long)0x99E00000;
	} else if (of_machine_is_compatible("sprd,sc9860g-1h10-3g")) {
		log_paddr = (unsigned long)0x94C00000;
	} else if (of_machine_is_compatible("sprd,sc9860g-2h10-3g")) {
		log_paddr = (unsigned long)0x94C00000;
	} else if (of_machine_is_compatible("sprd,sc9860g-3h10-3g")) {
		log_paddr = (unsigned long)0x94C00000;
	} else if (of_machine_is_compatible("sprd,sc9860g-2h10-native")) {
		log_paddr = (unsigned long)0x94C00000;
	} else if (of_machine_is_compatible("sprd,sc9860g-3h10-native")) {
		log_paddr = (unsigned long)0x94C00000;
	} else if (of_machine_is_compatible("mediatek,MT6735M")) {	
		log_paddr = (unsigned long)0x50100000;
	} else if (of_machine_is_compatible("mediatek,MT6750") \
			|| of_machine_is_compatible("mediatek,MT6755")) {	
