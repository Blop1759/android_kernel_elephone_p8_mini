...
static long emmc_rpmb_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	int ret = 0;
#if (defined(CONFIG_WD_TEE_SUPPORT))
	if (cmd == RPMB_IOCTL_WATCHDATA_CMD) {
		struct watchdata_rpmb_req rpmb_req, *rpmb_req_data;
		struct emmc_rpmb_req rpmb_req_rsp;
		uint32_t size;

		err = copy_from_user(&rpmb_req, (void *)arg, sizeof(struct watchdata_rpmb_req));
		if (err < 0) {
			MSG(ERR, "%s, err=%x\n", __func__, err);
			return -1;
		}

		MSG(INFO, "%s blk_cnt %d\n", __func__, rpmb_req.blk_cnt);
		size = sizeof(struct watchdata_rpmb_req) + sizeof(struct s_rpmb)*rpmb_req.blk_cnt;
		rpmb_req_data = (struct watchdata_rpmb_req *)kmalloc(size, GFP_KERNEL);
		if (rpmb_req_data == NULL) {
			MSG(ERR, "%s, err kmalloc failed, size %d, blk_cnt %d\n", __func__, size, rpmb_req.blk_cnt);
			return -1;
		}

		err = copy_from_user(rpmb_req_data, (void *)arg, size);
		if (err < 0) {
			MSG(ERR, "%s, err=%x\n", __func__, err);
			kfree(rpmb_req_data);
			return -1;
		...
		rpmb_req_rsp.type = rpmb_req_data->type;
		rpmb_req_rsp.blk_cnt = rpmb_req_data->blk_cnt;
		rpmb_req_rsp.data_frame = (u8 *)rpmb_req_data->data_frame;
#if 0
		{
			int cnt;
			for (cnt=0; cnt<rpmb_req_data->blk_cnt; cnt++) {
				wd_rpmb_dump_frame(&rpmb_req_rsp.data_frame[cnt]);
				dump_buf(&rpmb_req_rsp.data_frame[cnt], 512, "in rpmb driver");
			}

		}
#endif
		ret = emmc_rpmb_req_handle(card, &rpmb_req_rsp);
		if (ret) {
			MSG(ERR, "%s, emmc_rpmb_req_handle IO error!!!(%x)\n", __func__, ret);
			kfree(rpmb_req_data);
			return -1;
...
#if 0
		{
			...
			static const uint8_t rpmb_test_key[32] = {
				0x7A,0xFD,0xC2,0xA5,0xA9,0xDE,0x3B,0x44,
				0x25,0x91,0x43,0x7A,0xCD,0xAD,0xE3,0xDE,
				0x6E,0x37,0x78,0xAB,0xAA,0xFA,0xFE,0xA4,
				0xC3,0x4D,0x93,0xDC,0x6F,0xA7,0xBD,0xAE,
			};
			u8 hmac[RPMB_SZ_MAC];
			/*
			 * Authenticate response write counter frame.
			 */
			hmac_sha256(rpmb_test_key, 32, rpmb_req_data->data_frame->data, 284, hmac);

			dump_buf(hmac, 32, "out kernel hmac");
			if (memcmp(hmac, rpmb_req_data->data_frame->mac, RPMB_SZ_MAC) != 0) {
				MSG(ERR, "%s, hmac compare error!!!\n", __func__);
			}
			for (cnt=0; cnt<rpmb_req_data->blk_cnt; cnt++) {
				wd_rpmb_dump_frame(&rpmb_req_rsp.data_frame[cnt]);
				dump_buf(&rpmb_req_rsp.data_frame[cnt], 512, "out rpmb driver");
			}
		}
#endif
		ret = copy_to_user((void *)arg, rpmb_req_data, size);
		kfree(rpmb_req_data);
		return ret;
	}
#endif
