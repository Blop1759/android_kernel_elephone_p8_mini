			     ...
			     char* buffer)
{
 ...
 if (irq_node) {
      ...
      gpio_request(ints[0], "fpc");
      irq_state = gpio_get_value(ints[0]);
      pr_debug("%s line %d irq_state %d\n",__func__,__LINE__, irq_state);
    }
...
static int fpc1020_probe(struct platform_device *pldev)
{
	...
	dev_err(dev, "fpc1020_probe\n");
	//mt_spi_enable_clk();
	dev_err(dev, "spi enabled\n");
...
//	dev_err( dev, "Using GPIO#%d as IRQ.\n", fpc1020->irq_gpio );
	...
	dev_err( dev, "Using GPIO#%d as RST.\n", fpc1020->rst_gpio );
