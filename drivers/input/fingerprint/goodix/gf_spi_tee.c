...
static int gf_get_gpio_dts_info(struct gf_device *gf_dev)
{
	...
	node = of_find_compatible_node(NULL, NULL, "mediatek,vanzo-fp");
	if (node) {
...
/* Vanzo:yangbinbin on: Wed, 05 Apr 2017 16:58:03 +0800
...
// End of Vanzo: yangbinbin
    ...
    gf_dev->pins_ldo_1v8_off = pinctrl_lookup_state(gf_dev->pinctrl_gpios, "fp_ldo1v8_off");
    if (IS_ERR(gf_dev->pins_ldo_1v8_off)) {
        ret = PTR_ERR(gf_dev->pins_ldo_1v8_off);
        pr_info("%s can't find fingerprint pinctrl ldo_1v8_off\n", __func__);
        return ret;
    }
    gf_dev->pins_ldo_1v8_on = pinctrl_lookup_state(gf_dev->pinctrl_gpios, "fp_ldo1v8_on");
    if (IS_ERR(gf_dev->pins_ldo_1v8_on)) {
        ret = PTR_ERR(gf_dev->pins_ldo_1v8_on);
        pr_info("%s can't find fingerprint pinctrl ldo_1v8_on\n", __func__);
        return ret;
    }
    gf_dev->pins_ldo_3v3_off = pinctrl_lookup_state(gf_dev->pinctrl_gpios, "fp_ldo3v3_off");
    if (IS_ERR(gf_dev->pins_ldo_3v3_off)) {
        ret = PTR_ERR(gf_dev->pins_ldo_3v3_off);
        pr_info("%s can't find fingerprint pinctrl ldo_3v3_off\n", __func__);
        return ret;
    }
    gf_dev->pins_ldo_3v3_on = pinctrl_lookup_state(gf_dev->pinctrl_gpios, "fp_ldo3v3_on");
    if (IS_ERR(gf_dev->pins_ldo_3v3_on)) {
        ret = PTR_ERR(gf_dev->pins_ldo_3v3_on);
...
static void gf_hw_power_enable(struct gf_device *gf_dev, u8 onoff)
{
	...
	/* TODO:  set power  according to actual situation  */
		enable = 0;
		pinctrl_select_state(gf_dev->pinctrl_gpios, gf_dev->pins_ldo_1v8_on);
		pinctrl_select_state(gf_dev->pinctrl_gpios, gf_dev->pins_ldo_3v3_on);
		...
		pinctrl_select_state(gf_dev->pinctrl_gpios, gf_dev->pins_ldo_1v8_off);
		pinctrl_select_state(gf_dev->pinctrl_gpios, gf_dev->pins_ldo_3v3_off);
		enable = 1;
	}
}
...
static long gf_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	*/
	...
	case GF_IOC_INPUT_KEY_EVENT:
		...
		if (GF_KEY_HOME == gf_key.key) {
/* Vanzo:zhangweipeng on: Sat, 22 Apr 2017 15:26:49 +0800
 * implement #198013 modify fingerprint keycode
 */
                    key_input = KEY_F11;
...
/* Vanzo:yangbinbin on: Wed, 05 Apr 2017 16:59:27 +0800
 * TODO: replace this line with your comment
#ifdef CONFIG_OF
		if (flag == 0) {
...
{
/* Vanzo:yangbinbin on: Wed, 05 Apr 2017 16:58:58 +0800
 * TODO: replace this line with your comment
#ifdef CONFIG_OF
	if (pullhigh)
		pinctrl_select_state(gf_dev->pinctrl_gpios, gf_dev->pins_miso_pullhigh);
...
/* Vanzo:zhangweipeng on: Sat, 22 Apr 2017 15:27:20 +0800
 * implement #198013 modify fingerprint keycode
 */
...
/*-------------------------------------------------------------------------*/
static struct spi_board_info spi_board_gf_dev[] __initdata = {
    [0] = {
...
static int __init gf_init(void)
{
    ...
    spi_register_board_info(spi_board_gf_dev, ARRAY_SIZE(spi_board_gf_dev));
