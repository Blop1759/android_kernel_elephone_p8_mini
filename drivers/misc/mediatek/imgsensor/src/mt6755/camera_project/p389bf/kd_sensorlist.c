...
//Main2 support
#ifndef __DUAL_CAMERA_USEDBY_YUV_MODE__
	#define __DUAL_CAMERA_USEDBY_YUV_MODE__ 1
#endif
...
static u32 gI2CBusNum_main2 = SUPPORT_I2C_BUS_NUM3;
...
#if  __DUAL_CAMERA_USEDBY_YUV_MODE__
//from hct for p8_mini dual cam
extern SENSOR_FUNCTION_STRUCT_1	SensorFuncSP0A20YUV_1;
static SENSOR_FUNCTION_STRUCT_1 *g_pSensorFunc_1[1] = {&SensorFuncSP0A20YUV_1};
...
#if  __DUAL_CAMERA_USEDBY_YUV_MODE__
int SP0A20_iReadRegI2C(u8 *a_pSendData , u16 a_sizeSendData, u8 *a_pRecvData, u16 a_sizeRecvData, u16 i2cId)
{
    int  i4RetValue = 0;
...
int SP0A20_iWriteRegI2C(u8 *a_pSendData , u16 a_sizeSendData, u16 i2cId)
{
    int  i4RetValue = 0;
    int retry = 3;
...
#if  __DUAL_CAMERA_USEDBY_YUV_MODE__
{
	if ((CAMERA_DUAL_CAMERA_SENSOR_ENUM)g_invokeSocketIdx[i]==DUAL_CAMERA_MAIN_SENSOR)//i==0)//when it is main	poweron main2
		{
	/* wait for power stable */
	strcpy(buf_yuv_name, "sp0a20yuv");
	mDELAY(20);
	KD_IMGSENSOR_PROFILE("kdModulePowerOn");
	spin_lock(&kdsensor_drv_lock);
	gI2CBusNum = SUPPORT_I2C_BUS_NUM3;
	gI2CBusNum_main2 = SUPPORT_I2C_BUS_NUM3;
	spin_unlock(&kdsensor_drv_lock);
	/* turn on power */
	...
	printk("sp0a20yuv SensorGetID ret_main2:%d\n",ret_main2);
	/*if (ERROR_NONE != ret_main2) {
		j=1;
...
int kdSetDriver(unsigned int *pDrvIndex)
{
    ...
    /*  */
    if ((g_invokeSocketIdx[i]==2 || g_invokeSocketIdx[i]==1) && strcmp((char *)pSensorList[drvIdx[i]].drvname, "sp0a20yuv") == 0) {
		PK_XLOG_INFO("[kdSetDriver]dont use main2 for sub or main dirty version ... !\n");
		return -EIO;
	}
 ...
 */
#ifdef VANZO_DEVICE_NAME_SUPPORT
        {
          ...
          if(ERROR_NONE == err){
            ...
            }else if(DUAL_CAMERA_MAIN2_SENSOR==g_invokeSocketIdx[i]){
              v_set_dev_name(5, (char *)g_invokeSensorNameStr[i]);
            }
          }
        }
#endif
