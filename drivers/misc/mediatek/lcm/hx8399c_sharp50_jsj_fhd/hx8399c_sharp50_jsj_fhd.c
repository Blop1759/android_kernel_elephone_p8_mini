...
static unsigned int lcm_compare_id(void)
{
 ...
 #if defined(BUILD_LK)
         printf("LK 8399c-jsj adc = %x adcdata= %x %x, ret=%d, ADC_MIN_VALUE=%x\r\n",rawdata, adcdata[0], adcdata[1],ret, ADC_MIN_VALUE);
  #else
          printk("Kernel 8399c-jsj adc = %x adcdata= %x %x, ret=%d, ADC_MIN_VALUE=%x\r\n",rawdata, adcdata[0], adcdata[1],ret, ADC_MIN_VALUE);
  #endif
  rawdata=0;
	if(1)//((rawdata>0x100)) //0x63d
	{
		...
		id = id0<<8 | id1;
	#ifdef BUILD_LK
		printf("LK [hx8399]%s,  hsd id = 0x%x,0x%x,0x%x\n", __func__, id,id0,id1);
	#else
		printk("Kernel [hx8399]%s,  hsd id = 0x%x,0x%x,0x%x\n", __func__, id,id0,id1);
	#endif
...
LCM_DRIVER hx8399c_sharp50_jsj_fhd_lcm_drv = 
{
    .name			= "hx8399c_sharp50_jsj_fhd",
