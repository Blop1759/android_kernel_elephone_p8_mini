...
#define CAM_CALDB(fmt, arg...)    pr_warn("[%s] " fmt, __func__, ##arg)
...
static bool selective_read_byte(u32 addr, u8 *data, u16 i2c_id)
{
	...
	/*kdSetI2CSpeed(EEPROM_I2C_SPEED);*/
        ...
        CAM_CALDB("selective_read_byte addr =0x%x data = 0x%x \n", addr, *data);
	...
	unsigned char *data, unsigned int size)
{
	...
	if (selective_read_region(addr, data, g_pstI2Cclient->addr, size))
		return size;
	else
		return 0;
...
/*#define GT24C32_DRIVER_ON 0*/
#ifdef GT24C32_DRIVER_ON
#ifdef CONFIG_COMPAT
...
#endif
{
	...
	case CAM_CALIOC_G_READ:
		...
		GT24C32_DEVICE_ID, ptempbuf->u4Length);
