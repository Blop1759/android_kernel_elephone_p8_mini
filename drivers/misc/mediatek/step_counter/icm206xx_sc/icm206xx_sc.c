...
static int icm206xx_sc_WriteDMPImage(void)
{
	...
	res = icm206xx_share_write_memory(ICM206XX_DMP_CODE_START_LOC, 
	...
	while (nWritten < nSize)
	{
		...
		res = icm206xx_share_write_memory(nWritten, 
						nTryToWrite, 
...
static int icm206xx_sc_ReadDMPImage(void)
{
	...
	res = icm206xx_share_read_memory(ICM206XX_DMP_CODE_START_LOC, 
	...
	while (nRead < nSize)
	{
		...
		res = icm206xx_share_read_memory(nRead, 
						nTryToRead, 
...
static int icm206xx_sc_setDMPStartAddress(int addr)
{
	...
	res = icm206xx_share_write_register(ICM206XX_REG_PRGM_START_ADDRH, databuf, 2);
	if (res < 0) {
...
static int icm206xx_sc_startDMP(void)
{
	...
	databuf[0] = 0x80;
	res = icm206xx_share_write_register(ICM206XX_REG_USER_CTL, databuf, 1);
	if (res < 0) {
...
static int icm206xx_sc_EnableStepCounter(u8 en)
{
	...
	res = icm206xx_share_write_memory(ICM206XX_KEY_CFG_PED_ENABLE, 1, &databuf[0]);
	if (res < 0) {
	...
	if((en == 1) || 
	   ((en == 0) && icm206xx_share_any_step_sensor_is_on(ICM206XX_SENSOR_TYPE_STEP_COUNTER)))
	{
...
static int icm206xx_sc_EnableStepDetector(u8 en)
{
	int res = 0;
	u8 databuf[4] = {0};
	...
	res = icm206xx_share_write_memory(ICM206XX_KEY_CFG_PED_INT, 3, &databuf[0]);
	if (res < 0) {
		STEP_C_ERR("enable/disable step interrupt err!\n");
		return ICM206XX_ERR_BUS;
	}

	if((en == 1) || 
	   ((en == 0) && icm206xx_share_any_step_sensor_is_on(ICM206XX_SENSOR_TYPE_STEP_DETECTOR)))
		icm206xx_sc_startDMP();
	
	return res;
}

static int icm206xx_sc_EnableSMD(u8 en)
{
	int res = 0;
	u8 databuf[2] = {0};
	...
	res = icm206xx_share_write_memory(ICM206XX_KEY_SMD_ENABLE, 1, &databuf[0]);
	if (res < 0) {
		STEP_C_ERR("enable/disable SMD err!\n");
		return ICM206XX_ERR_BUS;
	}

	if((en == 1) || 
	   ((en == 0) && icm206xx_share_any_step_sensor_is_on(ICM206XX_SENSOR_TYPE_SIGNIFICANT_MOTION)))
		icm206xx_sc_startDMP();
...
static int icm206xx_sc_ReadSensorData(uint32_t *value)
{
	...
	res = icm206xx_share_read_memory(ICM206XX_KEY_D_PEDSTD_STEPCTR, 4, (u8 *)(&d));
	if (res < 0) {
...
//	STEP_C_LOG("Step Counter Data - %04x\n", (*value));
	printk("Step Counter Data - %04x\n", (*value));	

	return res;
}

void icm206xx_sc_NotifySensorData(int int_type)
{
	if((int_type & ICM206XX_DMP_INT_STEP_DETECTOR) == ICM206XX_DMP_INT_STEP_DETECTOR)
	{
		step_notify(TYPE_STEP_DETECTOR);
		STEP_C_LOG("Step Detector Nofity - %d\n", int_type);
	}

	if((int_type & ICM206XX_DMP_INT_SIGNIFICANT_MOTION) == ICM206XX_DMP_INT_SIGNIFICANT_MOTION)
	{
...
static int icm206xx_sc_setThresholdData(void)
{
	/* The default value stored in DMP is (sb, sb time, peak) = (5, 75, 2109832) */
	/* Current setting value is Vendor specific value (sb, sb time, peak) = (10, 100, 2516582) */
	/* Change Step Counter Thershold value after threshold tuning if it is required */

	int sb_threshold = 10;
	int sbt_threshold = 100;
	int peak_threshold = 2516582;

	int res = 0;
	u8 databuf[5] = {0};

	databuf[0] = (sb_threshold >> 8);
	databuf[1] = (sb_threshold & 0xFF);

	res = icm206xx_share_write_memory(D_PEDSTD_SB, 2, &databuf[0]);
	if (res < 0) {
		STEP_C_ERR("write Step Buffer err!\n");
		return 0;
	}

	databuf[0] = (sbt_threshold >> 8);
	databuf[1] = (sbt_threshold & 0xFF);

	res = icm206xx_share_write_memory(D_PEDSTD_SB_TIME, 2, &databuf[0]);
	if (res < 0) {
		STEP_C_ERR("write Step Buffer Time err!\n");
		return 0;
	}

	databuf[0] = (peak_threshold >> 24);
	databuf[1] = (peak_threshold >> 16);
	databuf[2] = (peak_threshold >> 8);
	databuf[3] = (peak_threshold & 0xFF);

	res = icm206xx_share_write_memory(D_PEDSTD_PEAKTHRSH, 4, &databuf[0]);
	if (res < 0) {
		STEP_C_ERR("write Peak threshold err!\n");
		return 0;
	}

	STEP_C_LOG("Set Step Counter Threshold ok!\n");
	return res;
}
...
static int icm206xx_sc_init_client(bool enable)
{
	...
	/* Set Step Counter Thershold value */
	icm206xx_sc_setThresholdData();		
	...
	/* Disable sensor and standby mode for accelerometer */
	res = icm206xx_share_EnableSensor(ICM206XX_SENSOR_TYPE_STEP_COUNTER, enable);
	if (res != ICM206XX_SUCCESS)
		return res;

	/* Disable sensor and standby mode for accelerometer */
	res = icm206xx_share_EnableSensor(ICM206XX_SENSOR_TYPE_STEP_DETECTOR, enable);
	if (res != ICM206XX_SUCCESS)
		return res;

	/* Disable sensor and standby mode for accelerometer */
	res = icm206xx_share_EnableSensor(ICM206XX_SENSOR_TYPE_SIGNIFICANT_MOTION, enable);
	if (res != ICM206XX_SUCCESS)
		return res;
...
static ssize_t show_sb_threshold(struct device_driver *ddri, char *buf)
{
	...
	res = icm206xx_share_read_memory(D_PEDSTD_SB, 2, (u8 *)(&d));
	if (res < 0) {
...
static ssize_t show_sbtime_threshold(struct device_driver *ddri, char *buf)
{
	...
	res = icm206xx_share_read_memory(D_PEDSTD_SB_TIME, 2, (u8 *)(&d));
	if (res < 0) {
		STEP_C_ERR("read Step Buffer Time err!\n");
		return 0;
	}
...
static ssize_t show_peak_threshold(struct device_driver *ddri, char *buf)
{
	...
	res = icm206xx_share_read_memory(D_PEDSTD_PEAKTHRSH, 4, (u8 *)(&d));
	if (res < 0) {
...
static int icm206xx_sc_enable_nodata(int en)
{
	...
	else 
	{
		...
		if(icm206xx_share_get_sensor_power(ICM206XX_SENSOR_TYPE_STEP_DETECTOR) == false)		
			icm206xx_sc_EnableStepCounter(en);
...
static int icm206xx_sc_enable_step_detect(int en)
{
	if(1 == en)
	{
		icm206xx_share_SetPowerMode(ICM206XX_SENSOR_TYPE_STEP_DETECTOR, true);
		icm206xx_share_EnableSensor(ICM206XX_SENSOR_TYPE_STEP_DETECTOR, true);
		icm206xx_share_SetSampleRate(ICM206XX_SENSOR_TYPE_STEP_DETECTOR, 5000000, false);

		icm206xx_share_EnableInterrupt(ICM206XX_BIT_DMP_INT_EN, en);

		icm206xx_sc_EnableStepCounter(en);
		icm206xx_sc_EnableStepDetector(en);
	}
	else
	{
		icm206xx_share_SetSampleRate(ICM206XX_SENSOR_TYPE_STEP_DETECTOR, 0, false);

		if(icm206xx_share_get_sensor_power(ICM206XX_SENSOR_TYPE_SIGNIFICANT_MOTION) == false)
			icm206xx_share_EnableInterrupt(ICM206XX_BIT_DMP_INT_EN, en);

		if(icm206xx_share_get_sensor_power(ICM206XX_SENSOR_TYPE_STEP_COUNTER) == false)
			icm206xx_sc_EnableStepCounter(en);

		icm206xx_sc_EnableStepDetector(en);

		icm206xx_share_EnableSensor(ICM206XX_SENSOR_TYPE_STEP_DETECTOR, false);
		icm206xx_share_SetPowerMode(ICM206XX_SENSOR_TYPE_STEP_DETECTOR, false);
	}

	STEP_C_LOG("icm206xx_sc_enable_step_detect OK!\n");
...
static int icm206xx_sc_enable_significant(int en)
{
	if(1 == en)
	{
		icm206xx_share_SetPowerMode(ICM206XX_SENSOR_TYPE_SIGNIFICANT_MOTION, true);
		icm206xx_share_EnableSensor(ICM206XX_SENSOR_TYPE_SIGNIFICANT_MOTION, true);
		icm206xx_share_SetSampleRate(ICM206XX_SENSOR_TYPE_SIGNIFICANT_MOTION, 5000000, false);

		icm206xx_share_EnableInterrupt(ICM206XX_BIT_DMP_INT_EN, en);
		icm206xx_sc_EnableSMD(en);
	}
	else
	{
		icm206xx_share_SetSampleRate(ICM206XX_SENSOR_TYPE_SIGNIFICANT_MOTION, 0, false);
		...
		icm206xx_share_EnableSensor(ICM206XX_SENSOR_TYPE_SIGNIFICANT_MOTION, false);
		icm206xx_share_SetPowerMode(ICM206XX_SENSOR_TYPE_SIGNIFICANT_MOTION, false);
	}
	
	STEP_C_LOG("icm206xx_sc_enable_significant OK!\n");
...
static int icm206xx_sc_get_data_step_c(uint32_t *value, int *status)
{
...
static int icm206xx_sc_attr_create(void)
{
	...
	data.get_data = icm206xx_sc_get_data_step_c;			// function pointer
